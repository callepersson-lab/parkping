...
export function HomeScreen({ navigation }: HomeScreenProps): React.JSX.Element {
  ...
  const handleOpenLastParkingMap = () => {
    if (!lastParking) return;

    navigation.navigate("LastParkingMap", {
      latitude: lastParking.latitude,
      longitude: lastParking.longitude,
    });
  };

  // Show map icon when monitoring and we have a last parking
  const showMapIcon = state === "monitoring" && lastParking != null;
        ...
        <View style={[styles.statusContainer, { borderColor: stateColor }]}>
          {showMapIcon ? (
            <TouchableOpacity
              ...
              onPress={handleOpenLastParkingMap}
              activeOpacity={0.8}
            >
              <MapPinned color={stateColor} size={18} />
            </TouchableOpacity>
          ) : (
